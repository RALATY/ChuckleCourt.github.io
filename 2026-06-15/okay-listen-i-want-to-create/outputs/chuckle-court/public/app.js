...
function settingsPanel(room) {
  return `
    ...
    <div class="toggle-row" style="margin-top:10px">
      <div>
        <strong>Photo-only minigames</strong>
        <div class="muted">When enabled, minigames will always be the photo/caption minigame.</div>
      </div>
      <label class="switch" aria-label="Toggle photo-only">
        <input type="checkbox" data-action="toggle-photo-only" ${room.settings.photoOnly ? "checked" : ""} />
        <span class="slider"></span>
      </label>
    </div>
  `;
...
function stageView(room, round) {
  if (round.stage === "questioning") {
    const panel = submitPanel("Write a question that will be sent to someone else.", "Make it weird, short, and dangerously answerable.", round.submitted);
    const hostForce = room.isHost ? `<div style="margin-top:8px" class="host-controls"><button data-action="force-next" class="secondary">Force next (host)</button></div>` : "";
    return panel + hostForce;
  }
  if (round.stage === "answering") {
    const panel = submitPanel("Write your answer.", round.kind === "questionAnswer" ? "Your name stays hidden in this minigame." : "After voting, the court reveals who wrote what.", round.submitted);
    const hostForce = room.isHost ? `<div style="margin-top:8px" class="host-controls"><button data-action="force-next" class="secondary">Force next (host)</button></div>` : "";
    return panel + hostForce;
  }
...
function resultsPanel(room, round) {
  return `
    <section class="panel tint-yellow stack">
      <div class="phase-line">
        <h2>Verdict</h2>
        ${room.isHost ? `<div><button data-action="next-round">Next</button></div>` : `<span class="status-badge blue">Waiting for host</span>`}
      </div>
      <div class="options-list">
...
app.addEventListener("click", async (event) => {
  ...
  try {
    if (action === "home") {
    ...
    } else if (action === "toggle-photo-only") {
      await roomAction("/api/settings", { photoOnly: button.checked });
      ...
      if (!state.room?.isHost) return showToast("Only the host can force progression.");
      if (!round || !["questioning","answering"].includes(round.stage)) return showToast("Force-next is only allowed during the typing phase.");
